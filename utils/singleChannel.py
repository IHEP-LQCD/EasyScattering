...
class ScatteringSingleChannelCalculator:
    ...
    def conv2CM(self, mass1: float, mass2: float, pa: List[int], pb: List[int], E_L_obs):
        pa = np.asarray(pa)
        pb = np.asarray(pb)
        self.d = pa + pb
        ...
        p_CM_sq = (
            ...
            + mass2**4
            - 2 * mass1**2 * mass2**2
            - 2 * E_CM_obs**2 * mass1**2
            - 2 * E_CM_obs**2 * mass2**2
        ) / (2 * E_CM_obs) ** 2
        E_L_thres = np.sqrt(np.sum(pa**2) * p2fac + mass1**2) + np.sqrt(np.sum(pb**2) * p2fac + mass2**2)
        print(f"pa = {pa}, pb = {pb}")
        print(
    ...
    def E_CM_from_thres(
        ...
        p_CM_sq = (
            ...
            + mass2**4
            - 2 * mass1**2 * mass2**2
            - 2 * E_CM_from_thres**2 * mass1**2
            - 2 * E_CM_from_thres**2 * mass2**2
        ) / (2 * E_CM_from_thres) ** 2
        q2 = p_CM_sq / p2fac
        # print(F"pa = {pa}, pb = {pb}")
        # print(
    ...
    # @staticmethod
    def factory_pcotdelta_fast_restframe(self, l1: int, m1: int, l2: int, m2: int):
        if l1 != 0 or l2 != 0:
        ...
        Z3_cut = self.cut_off
        Z3_space = np.array(list(product(range(-Z3_cut, Z3_cut + 1), repeat=3)), dtype="f8")
        gamma = 1
        gamma_inv = 1 / gamma
        Z3_cut2 = Z3_cut**2
        ...
        def pcotdelta(k2):
            ...
            sum0 = np.exp(lam * (q2 - n2)) / (n2 - q2) / np.sqrt(4 * np.pi) + gamma * np.sqrt(np.pi) / 4 * (
                np.exp(-2 * np.pi * n_abs_lam * c_minus_q2)
                * (
                    2
                    - special.erfc(c_minus_q2 - np.pi * n_abs_lam)
            ...
            Zd00 += np.exp(lam * (q2 - z_0_sq)) / (z_0_sq - q2) / np.sqrt(4 * np.pi)
            if k2 >= 0:
                Zd00 += (
                    gamma * np.pi * (np.exp(lam * q2) * (2 * np.sqrt(lam * q2) * special.dawsn(np.sqrt(lam * q2)) - 1))
                )
            else:
                Zd00 += (
                    gamma * np.pi * (np.exp(lam * q2) - np.sqrt(-np.pi * lam * q2) * special.erf(np.sqrt(-lam * q2)))
                )
            pcotdelta0 = 2 * Zd00 / (np.sqrt(np.pi) * L * xi_0 / a_t_inv_GeV * gamma)
            # print(f"pcotdelta = {pcotdelta0}")
            return pcotdelta0

        return pcotdelta

    def factory_pcotdelta_fast_flight(self, l1: int, m1: int, l2: int, m2: int):
        if l1 != 0 or l2 != 0:
        ...
        def pcotdelta(k2, lam: float = 1):
            ...
            term2 = (
                ...
                * (
                    ...
                    * (
                        2
                        - special.erfc(c_minus_q2 - np.pi * w_abs_lam)
            ...
            term3 = np.exp(lam * (q2 - z_0_sq)) / (z_0_sq - q2) / np.sqrt(4 * np.pi)
            Zd00 += term3
            if k2 >= 0:
                term4 = (
                    gamma * np.pi * (np.exp(q2 * lam) * (2 * np.sqrt(q2 * lam) * special.dawsn(np.sqrt(q2 * lam)) - 1))
                )
            else:
                term4 = (
                    gamma * np.pi * (np.exp(q2 * lam) - np.sqrt(-np.pi * q2 * lam) * special.erf(np.sqrt(-q2 * lam)))
                )
            Zd00 += term4
            pcotdelta0 = Zd00 * 2 / (np.sqrt(np.pi) * L * xi_0 / a_t_inv_GeV * gamma)
